...
import sdfascii
from sdfascii.__main__ import main


class TestReadingSDFFormat(unittest.TestCase):
    def setUp(self):
        source_10mvrms_3khz_directory = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "source_10mVrms_3kHz"
        )

        sdf_file = os.path.join(source_10mvrms_3khz_directory, "SDF3KHZ.DAT")

        ascii_ydata_file = os.path.join(source_10mvrms_3khz_directory, "ASCII3KH.TXT")
    ...
    def test_reading_sdf_file_identifier(self):
        self.assertTrue(
            self.sdf_hdr["valid_file_identifier"], "Invalid SDF file identifier"
        )
    ...
    def test_sdf_file_hdr_application_version(self):
        self.assertEqual(self.sdf_hdr["file_hdr"]["application_version"], "A.01.11")
    ...
    def test_sdf_data_hdr_data_type(self):
        self.assertEqual(
            self.sdf_hdr["data_hdr"][0]["data_type"], "Auto-power spectrum"
        )
    ...
    def test_sdf_channel_hdr_channel_label(self):
        self.assertEqual(self.sdf_hdr["channel_hdr"][0]["channel_label"], "Chan  1")
        self.assertEqual(self.sdf_hdr["channel_hdr"][1]["channel_label"], "Chan  1")
    ...
    def test_sdf_channel_hdr_window(self):
        self.assertEqual(
            self.sdf_hdr["channel_hdr"][0]["window"]["window_type"], "Flat Top"
        )
        self.assertEqual(
        ...
        self.assertAlmostEqual(
            self.sdf_hdr["channel_hdr"][0]["window"]["bw"], 3.81935954
        )
        ...
        self.assertAlmostEqual(
            self.sdf_hdr["channel_hdr"][0]["window"]["wide_band_corr"], 2.39823508262
        )
        self.assertAlmostEqual(
            self.sdf_hdr["channel_hdr"][0]["window"]["narrow_band_corr"], 4.68691444
        )
    ...
    def test_sdf_channel_hdr_channel_attribute(self):
        self.assertEqual(
            self.sdf_hdr["channel_hdr"][0]["channel_attribute"], "No attribute"
        )
...
class TestReadingASCIIFormat(unittest.TestCase):
    def setUp(self):
        ...
        ascii_file_basename = os.path.join(source_10mvrms_3khz_directory, "ASCII3KH")
    ...
    def test_all_frequencies(self):
        ...
        np.testing.assert_array_equal(self.ascii_data["frequency"], frequency_answer)
...
class TestCommandLineInterface(unittest.TestCase):
    """`python -m sdfascii` is what the examples/ JSON is regenerated with."""
    ...
    def test_writes_the_header_as_json(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            output = os.path.join(tmpdir, "out.json")
            main(["sdf", self.sdf_file, output])
            with open(output) as json_file:
                header = json.load(json_file)
        self.assertEqual(header["file_hdr"]["application"], "HP 35670A")
        self.assertEqual(header["file_hdr"]["sdf_revision"], 2)

    def test_rejects_a_filetype_it_cannot_read(self):
        # `ascii` used to be accepted here and then silently do nothing.
