...
"""Read Standard Data Format (SDF) and ASCII files created by HP/Agilent
...
# The revision of the SDF format a file was written to, from the sdf_revision
# field of its file header record. Only revision 2 is read in full; the
# revision 1 and 3 branches are placeholders.
SDF_REVISION_1 = 1
SDF_REVISION_2 = 2
SDF_REVISION_3 = 3
...
def _decode_sdf_unit(binary_data: bytes) -> SDFUnit:
    ...
    unit_dict = dict(zip(keys, values, strict=True))
...
def _decode_sdf_window(binary_data: bytes) -> SDFWindow:
    ...
    window_dict = dict(zip(keys, values, strict=True))
    window_type_decoder = {
    ...
    window_dict["window_type"] = window_type_decoder[window_dict["window_type"]]
    correction_mode_decoder = {
...
def read_ascii_files(input_ascii_base_filename):
    ...
    return np.rec.fromarrays([xdata, ydata], names="frequency,amplitude")


def _decode_sdf_file_hdr(
    record_size: int, binary_data: bytes
) -> SDFFileHdrV1 | SDFFileHdrV2 | SDFFileHdrV3:
    ...
    file_hdr: dict[str, int | str | datetime] = {}
    ...
    (
        file_hdr["num_data_hdr_records"],
        file_hdr["num_vector_hdr_records"],
        file_hdr["num_channel_hdr_records"],
        file_hdr["num_unique_records"],
        file_hdr["num_scan_struct_records"],
        file_hdr["num_xdata_records"],
    ) = struct.unpack(">6h", binary_data[24:36])
    (
        file_hdr["offset_data_hdr_record"],
        file_hdr["offset_vector_record"],
        file_hdr["offset_channel_record"],
        file_hdr["offset_unique_record"],
        file_hdr["offset_scan_struct_record"],
        file_hdr["offset_xdata_record"],
        file_hdr["offset_ydata_record"],
    ) = struct.unpack(">7l", binary_data[36:64])

    if file_hdr["sdf_revision"] == SDF_REVISION_1:
        temp_file_hdr = cast(SDFFileHdrV1, file_hdr)
    elif file_hdr["sdf_revision"] == SDF_REVISION_2:
        temp_file_hdr = cast(SDFFileHdrV2, file_hdr)
    elif file_hdr["sdf_revision"] == SDF_REVISION_3:
...
def _decode_sdf_meas_hdr(
...
) -> SDFMeasHdrV1 | SDFMeasHdrV2 | SDFMeasHdrV3:
    """
    ...
    (meas_hdr["center_freq"], meas_hdr["span_freq"], meas_hdr["sweep_freq"]) = (
        struct.unpack(b">3d", binary_data[102:126])
    )
    ...
    if sdf_revision == SDF_REVISION_1:
        # Decode the revision 1 stuff
        # FIXME: Add rev 1 stuff later
        temp_meas_hdr = cast(SDFMeasHdrV1, meas_hdr)
    elif sdf_revision == SDF_REVISION_2:
        # Decode the revision 2 stuff
        (meas_hdr["start_freq_index"], meas_hdr["stop_freq_index"]) = struct.unpack(
            b">2h", binary_data[24:28]
        )
        temp_meas_hdr = cast(SDFMeasHdrV2, meas_hdr)
    elif sdf_revision == SDF_REVISION_3:
...
# A flat walk through the fields of one record, in the order the Standard Data
# Format Utilities User's Guide lists them. Splitting it to satisfy the
# statement count would scatter a description of the format that reads in one
# piece, so the limit is waived here rather than answered.
def _decode_sdf_data_hdr(  # noqa: PLR0915
    record_size: int, sdf_revision: int, binary_data: bytes
) -> SDFDataHdrV1 | SDFDataHdrV2 | SDFDataHdrV3:
    """
    Decode the data header binary data
    """
    data_hdr: dict = {}
    data_hdr["record_size"] = record_size
    (data_hdr["offset_unique_record"],) = struct.unpack(">1l", binary_data[6:10])
    ...
    if sdf_revision == SDF_REVISION_1:
        # Decode the revision 1 stuff
        # FIXME: Add rev 1 stuff later
        (data_hdr["abscissa_first_x"], data_hdr["abscissa_delta_x"]) = struct.unpack(
            ">2f", binary_data[34:42]
        )
        temp_data_hdr = cast(SDFDataHdrV1, data_hdr)
    elif sdf_revision == SDF_REVISION_2:
        # Decode the revision 2 stuff
        (data_hdr["num_points"], data_hdr["last_valid_index"]) = struct.unpack(
            ">2h", binary_data[30:34]
        )
        (data_hdr["abscissa_first_x"], data_hdr["abscissa_delta_x"]) = struct.unpack(
            b">2d", binary_data[114:130]
        )
...
def _decode_sdf_vector_hdr(
    ...
    (vector_hdr["offset_unique_record"],) = struct.unpack(">l", binary_data[6:10])
...
def _decode_sdf_channel_hdr(
    ...
    (channel_hdr["offset_unique_record"],) = struct.unpack(">l", binary_data[6:10])
    ...
    channel_hdr["channel_attribute"] = channel_attribute_decoder[
        coded_channel_attribute
    ]
    ...
    (
        channel_hdr["channel_scale"],
        channel_hdr["channel_offset"],
        channel_hdr["gate_begin"],
        channel_hdr["gate_end"],
        channel_hdr["user_delay"],
    ) = struct.unpack(b">5d", binary_data[152:192])
...
def _decode_sdf_scan_struct(
    ...
    scan_var_type_decoder = {1: "Short", 2: "Long", 3: "Float", 4: "Double"}
...
# Reads the file's records in the order they appear on disk, one after another,
# each guarded by a check on the record type it should be. The branch and
# statement counts are what walking a whole file format in one pass costs; the
# same reasoning as _decode_sdf_data_hdr applies.
def read_sdf_file(sdf_filename: str) -> tuple[Any, Any]:  # noqa: PLR0912, PLR0915
    ...
    with open(sdf_filename, "rb") as sdf_file:
        ...
        file_hdr_record_type, file_hdr_record_size = struct.unpack(
            ...
            sdf_file.read(struct.calcsize(record_type_size_format.encode())),
        )
        ...
        sdf_hdr["file_hdr"] = _decode_sdf_file_hdr(
            file_hdr_record_size, sdf_file.read(file_hdr_record_size)
        )
        ...
        for data_hdr_record_index in range(sdf_hdr["file_hdr"]["num_data_hdr_records"]):
            ...
            sdf_file.seek(
                sdf_hdr["file_hdr"]["offset_data_hdr_record"]
        ...
        for vector_hdr_record_index in range(
            ...
            sdf_file.seek(
                sdf_hdr["file_hdr"]["offset_vector_record"]
            ...
            sdf_file.seek(sdf_file.tell() - struct.calcsize(record_type_size_format))
            sdf_hdr["vector_hdr"].append(
                _decode_sdf_vector_hdr(
        ...
        for channel_hdr_record_index in range(
            ...
            sdf_file.seek(
                sdf_hdr["file_hdr"]["offset_channel_record"]
            ...
            # the record type and record size
            sdf_file.seek(sdf_file.tell() - struct.calcsize(record_type_size_format))
            sdf_hdr["channel_hdr"].append(
                _decode_sdf_channel_hdr(
        ...
        for scan_struct_record_index in range(
            ...
            sdf_file.seek(
                sdf_hdr["file_hdr"]["offset_scan_struct_record"]
        ...
        # ------------------------------------------------------------------- #
        # Bound here rather than only inside the branch below, which is what
        # the guard exists to skip: a file with no y-data record used to reach
        # the return statement with nothing assigned and raise
        # UnboundLocalError.
        sdf_data = None
        if sdf_hdr["file_hdr"]["offset_ydata_record"] >= 0:
            ...
            # Read the record type and size
            yaxis_data_record_type, _yaxis_data_record_size = struct.unpack(
                record_type_size_format,
                sdf_file.read(struct.calcsize(record_type_size_format)),
            )
            ...
            if resp_ch_id == -1:
                ...
                if (
                    data_hdr["domain"] == "Frequency domain"
                    or data_hdr["domain"] == "Channel"
                ):
                    resp_window_corr = resp_ch["window"]["narrow_band_corr"]
                else:
                    resp_window_corr = 1.0

                # Calculate te combined correction factor for the response
                # channel.
                resp_ch_corr_factor = (resp_window_corr / resp_eu_corr) ** (
                    pwr_of_resp_ch / 48
                )
            ...
            if exciter_ch_id == -1:
                ...
                if (
                    ...
                    exciter_window_corr = exciter_ch["window"]["narrow_band_corr"]
                else:
                    exciter_window_corr = 1.0

                # Calculate te combined correction factor for the exciter
                # channel.
                exciter_ch_corr_factor = (exciter_window_corr / exciter_eu_corr) ** (
            ...
            if data_hdr["y_is_complex"]:
                dtype = np.dtype(">c8")
                count *= 2
            sdf_data = np.fromfile(file=sdf_file, dtype=dtype, count=count, sep="")
            ...
            meas_hdr = cast(SDFMeasHdrV2, sdf_hdr["meas_hdr"])
            start_idx = meas_hdr["start_freq_index"]
            stop_idx = meas_hdr["stop_freq_index"]
